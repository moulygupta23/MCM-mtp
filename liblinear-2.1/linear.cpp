...
double printx(const feature_node *x)
{
	int j=0;
	while(x->index != -1 && j<4)
	{
...
static void solve_l2r_l1l2_mcm(
	const problem *prob, double *w, double eps,
	double Cp, double Cn, int solver_type)
{
	int l = prob->l;
	...
	while (iter < max_iter)
	{
		...
		// 		sumalpha+=alpha[i];
		// 		sumbeta+=beta[i];
		// 		v += (-beta[i]*2 + 2*lambda*alpha[i] + diag[GETI(i)]*(beta[i]*beta[i]+alpha[i]*alpha[i]));
		// 		// std::cout<<alpha[i]<<" "<<beta[i]<<std::endl;
		// 	}
		// 	v+= -2*lambda;
		// 	std::cout<<iter;
		// 	std::cout<<"\n"<<removedalpha<<" "<<removedbeta;
		// 	std::cout<<"\nObjective value = "<<v/2<<"\n";
		// 	std::cout<<"sum of alpha = "<<sumalpha<<"\nsum of beta = "<<sumbeta<<"\n";
...
double predict_values(const struct model *model_, const struct feature_node *x, double *dec_values)
{
	...
	if(nr_class==2)
	{
		// std::cout<<dec_values[0]<<std::endl;
		if(check_regression_model(model_))
			return dec_values[0];
		else
